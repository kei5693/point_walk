...
let main = {
  ...
  mainScrollEvent: function(){
    ...
    function resetMainScrollEvent(){
      ...
      mainTextSwipe.autoplay.stop(); // 2023-11-30 수정
...
let challenge = {
  ...
  challengeRewardEvent: function () {
    ...
    window.addEventListener('scroll', () => {
      const currentScrollPos = window.pageYOffset || document.documentElement.scrollTop;
      let calcPos = challengeReward.offsetTop - currentScrollPos;

      if(calcPos < 116){
        // 재실행 방지
        if(rewardBtn.classList.contains('active')){
          setListHeight(rewardBtn, rewardListHeight);
        }
      }
    });
    ...
    rewardBtn.addEventListener('click', (e) => {
      ...
      common.scrollToEvent('html', challengeReward, window.outerHeight/2);
      setTimeout(() => {
